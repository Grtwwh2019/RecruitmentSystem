...
<div class="JobTab">
  ...
  <div class="hotJob">
        ...
        <ul class="jobList">
            <li v-for="job in jobList" :key="job.id">
              <!-- <router-link :to="'/jobsDetail/'+JSON.stringify(job)"> -->
            <!-- <router-link :to="{path:'/JobsDetail', query: {job: job}}"> -->
            <div style="cursor: pointer;" @click="toJobDetail(job)">
              <div class="job_top">
                    <div class="jtop_t">
                        <h3>{{job.ename}}</h3>
                        <span>[{{job.createTime}} 发布]</span>
                        <!-- <i class="iconfont icon-icon_xinxi"></i> -->
                        <span class="money">{{job.esalaryfloor}}-{{job.esalaryceilie}}</span>
                    </div>
                    <div class="jtop_m">应届毕业生 / {{degree[job.educationrequire]}}</div>
                ...
                <div class="job_bot">
                    ...
                    <div class="job_name">
                        <p class="title">{{job.ecompany}}</p>
                        <p class="msg">{{job.industry}} / {{financing[job.financing]}} / {{companySize[job.csize]}} / {{job.ecity}}</p>    
                    </div>
                </div>
              </div>
            <!-- </router-link> -->
        ...
        <router-link to="/searchList" class="show_more">查看更多</router-link>
   </div>
   <!-- <div class="jobTab">
    <ul>
  ...
  <div class="fslink">
   ...
   <div v-if="friendsLink.length > 10" class="switch" @click="showToggle">
...
<script>
  ...
  import { degree, companySize, financing, friendsLink } from "@/const/index";
  ...
  import HotCompany from '@/components/base/HotCompany'
  export default {
  name: 'JobTab',
  components: {
      HotCompany
      ...
      searchList(params).then(resp => {
        if (resp && resp.status == 0) {
          if (resp.data.total > 9) {
            this.jobList = resp.data.list.splice(0, 9)
          } else {
            this.jobList = resp.data.list
          }
          this.jobList.forEach(element => {
            let date = element.createTime.split("-")
            element.createTime = date[1] + "-" + date[2]
          });
        }
      })
    },
